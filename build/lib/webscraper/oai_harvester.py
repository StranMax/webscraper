...
class Downloader():
    ...
    def download_file(self, url):
        ...
        #    filename_valid = "".join(x for x in s if x.isalnum())
        #    logging.debug(f'Converted {filename} to {filename_valid}')
        ...
        for attempt in range(1,3):
            ...
            try:
                ...
                with requests.get(url, stream=True) as response:
                    ...
                    if out_file.exists(): 
                        logging.info(f'File exists: {out_file}. Skip.')
                        return None
                    ...
                    logging.info(f'Downloaded {filename_valid}')
                    return None
                    
            except Exception as ex:
                logging.warning(f'Attempt #{attempt} failed with error: {ex}')
